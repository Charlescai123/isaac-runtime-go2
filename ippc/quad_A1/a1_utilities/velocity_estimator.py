"""Code source: https://github.com/ikostrikov/walk_in_the_park/blob/main/real/robots/a1_robot_velocity_estimator.py"""
"""Estimates base velocity for A1 robot from accelerometer readings."""
import numpy as np
from filterpy.kalman import KalmanFilter
from quad_A1.a1_utilities.moving_window_filter import MovingWindowFilter
from quad_gym import gym_config
  ...
  """
    ...
    def _compute_delta_time(self, current_time):
        if self._last_timestamp == 0.:
            ...
            delta_time_s = self.gym_params.SimParams.env_time_step
        else:
            delta_time_s = current_time - self._last_timestamp
        self._last_timestamp = current_time
        return delta_time_s

    def update(self, current_time, observation):
        """Propagate current state estimate with new accelerometer reading."""
        delta_time_s = self._compute_delta_time(current_time)
        sensor_acc = observation.imu.accelerometer  # dim :3
        self._raw_acc = sensor_acc
        base_orientation = observation.imu.quaternion # dim: 3
        rot_mat = self.robot.pybullet_client.getMatrixFromQuaternion(base_orientation)  # todo call function directly
        ...
        for i in range(12):
            motor_velocities[i] = observation.motorState[i].dq
