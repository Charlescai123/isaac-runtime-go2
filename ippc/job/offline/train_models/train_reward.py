...
import random
import pickle
from job.offline.models.reward_model import RewardModel
...
def train_reward(seed=100, device="cuda"):
    ...
    epoch_num = 8000
    reward_model = RewardModel(state_dim=state_dim).to(device=device)
    optimizer = torch.optim.Adam(reward_model.parameters(), lr=0.0001)
    ...
    for i in range(epoch_num):
        ...
        reward_pred = reward_model(s_comb)
        loss = loss_fn(reward_pred, reward_true)
        print(f"loss at epoch {i}", loss)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()

    torch.save(reward_model.state_dict(), "job/offline/weights/reward_model.pt")
