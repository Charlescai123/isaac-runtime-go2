...
def train_agent(params, agent, env, logger):
    ...
    while logger.global_steps <= agent_params.total_timesteps:
        ...
        for step in range(agent_params.num_steps):
            ...
            if logger.global_steps > agent_params.learning_starts:  # step-wise optimization
        ...
        if logger.global_steps > agent_params.learning_starts:
            ...
            print(f"Prefilling replay memory using random actions, Current progress {logger.global_steps} / "
                  f"{int(agent_params.learning_starts)}")
