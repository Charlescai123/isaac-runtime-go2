...
def train_agent(params, agent, env, logger):
    ...
    while logger.global_steps <= agent_params.total_timesteps:
        ...
        for step in range(agent_params.num_steps):
            ...
            actions, log_prob, value = agent.get_actions(obs, mode='train')

            next_obs, rewards, terminations, _, infos = env.step(actions)

            agent.replay_buffer.add(obs, actions, rewards, log_prob, terminations, value)

            obs = next_obs

            logger.log_step_performance_info(rewards, env.unwrapped.get_states_robot())

            if agent.replay_buffer.get_size() == agent_params.num_steps:
                loss_dict = agent.train_step()
                logger.log_step_loss_info(loss_dict)
                agent.replay_buffer.reset_online_buffer()
                break
